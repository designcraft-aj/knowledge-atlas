...
- A year slider — only artists alive in that year appear
- Clicking a portrait shows a detail card (Stage 3, not yet built)

## File structure
    public/
      data/
        people.json        ← artist database
        movements.json     ← art movement names + year ranges
      assets/
        portraits/         ← portrait images, empty for now
    src/
      main.js              ← all JavaScript
      style.css            ← all CSS
    index.html             ← entry point
    enrich.py              ← Wikipedia enrichment, built later
    CLAUDE.md              ← this file

## Rules — never break these
- Vanilla JS only, no JS framework
- D3 is installed via npm, import it in main.js
- Data loaded via fetch() from /data/people.json and /data/movements.json
